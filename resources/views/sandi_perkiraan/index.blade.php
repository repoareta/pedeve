		...
		<div class="kt-subheader__main">
			...
			<div class="kt-subheader__breadcrumbs">
				...
				<a href="" class="kt-subheader__breadcrumbs-link">
					Kontroler </a>
				<span class="kt-subheader__breadcrumbs-separator"></span>
				<span class="kt-subheader__breadcrumbs-link kt-subheader__breadcrumbs-link--active">Sandi Perkiraan</span>
			</div>
		</div>
	</div>
</div>
...
$(document).ready(function () {
		var t = $('#kt_table').DataTable({
			...
			ajax      : {
						url: "{{ route('sandi_perkiraan.search.index') }}",
						type : "POST",
						dataType : "JSON",
						headers: {
			...
			if($('input[class=btn-radio]').is(':checked')) { 
				$("input[class=btn-radio]:checked").each(function() {
					...
					const swalWithBootstrapButtons = Swal.mixin({
						...
						if (result.value) {
							$.ajax({
								url: "{{ route('sandi_perkiraan.delete') }}",
								type: 'DELETE',
								dataType: 'json',
								data: {
									"kode": kode,
		...
		$('#editRow').click(function(e) {
			...
			if($('input[class=btn-radio]').is(':checked')) { 
				$("input[class=btn-radio]:checked").each(function(){
					...
					location.replace("{{url('kontroler/sandi_perkiraan/edit')}}"+ '/' +no);
				});
			} else {
				swalAlertInit('ubah');
			}
