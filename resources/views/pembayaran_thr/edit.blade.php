					...
					<div class="form-group form-group-last">
						<div class="alert alert-secondary" role="alert">
							<div class="alert-text">
								<h5 class="kt-portlet__head-title">
									Header Menu Edit Pembayaran THR
								</h5>	
							</div>
						</div>
						...
						<div class="kt-form__actions">
							<div class="row">
								<div class="col-2"></div>
								<div class="col-10">
									<a  href="{{route('pembayaran_thr.index')}}" class="btn btn-warning"><i class="fa fa-reply" aria-hidden="true"></i>Cancel</a>
...
$.ajax({
	url : "{{route('pembayaran_thr.lokasiJson')}}",
	type : "POST",
	dataType: 'json',
	data : {
		jk:jk,
...
$('#form-edit').submit(function(){
	$.ajax({
		url  : "{{route('pembayaran_thr.update')}}",
		type : "POST",
		...
		Swal.fire({
			...
			timer : 2000
				...
				window.location.replace("{{ route('pembayaran_thr.index') }}");;
			});
		}, 
		error : function(){
...
var bulanbuku = $('#bulanbuku').val();
	$.ajax({
		url : "{{route('pembayaran_thr.createJson')}}",
		type : "POST",
		dataType: 'json',
		data : {
			bagian:bagian,
...
$("#lokasi").on("click", function(){
	...
	var tahun = $('#tahun').val();
		...
		$.ajax({
			url : "{{route('pembayaran_thr.nobuktiJson')}}",
			type : "POST",
			dataType: 'json',
			data : {
				lokasi:lokasi,
...
$('#btn-create').on('click', function(e) {
	e.preventDefault();
	$('#title-detail').html("Tambah Detail Pembayaran THR");
	$('.modal-create').modal('show');
});
$('#btn-delete-all').on('click', function(e) {
	e.preventDefault();
	$('#title-delete-detail').html("Hapus Detail Pembayaran THR All");
	...
	$('#form-create-detail').submit(function(){
		$.ajax({
			url  : "{{route('pembayaran_thr.store.detail')}}",
			type : "POST",
	...
	$('#form-delete-all').submit(function(){
		$.ajax({
			url  : "{{route('pembayaran_thr.delete.detail.all')}}",
			type : "delete",
...
if($('input[type=radio]').is(':checked')) {  
	$("input[type=radio]:checked").each(function() {  
		...
		var nourut = $(this).attr('nourut');
			$.ajax({
				url :"{{('perbendaharaan/pembayaran_thr/editdetail')}}"+ '/' +nodok+ '/' +nourut,
				type : 'get',
				dataType:"json",
				headers: {
					'X-CSRF-Token': '{{ csrf_token() }}',
				...
				success:function(data)
				{
					...
					$('#nilai1').val(output);
					$('#title-edit-detail').html("Edit Detail Pembayaran THR");
...
$('#form-edit-detail').submit(function(){
		$.ajax({
			url  : "{{route('pembayaran_thr.update.detail')}}",
			type : "POST",
			...
			if($('input[type=radio]').is(':checked')) { 
				$("input[type=radio]:checked").each(function() {
					...
					.then((result) => {
						if (result.value) {
							$.ajax({
								url: "{{ route('pembayaran_thr.delete.detail') }}",
								type: 'DELETE',
								dataType: 'json',
								data: {
									"nodok": nodok,
