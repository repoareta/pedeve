		...
		<div class="kt-subheader__main">
			...
			<div class="kt-subheader__breadcrumbs">
				...
				<a href="" class="kt-subheader__breadcrumbs-link">
					Kontroler </a>
				<span class="kt-subheader__breadcrumbs-separator"></span>
				<span class="kt-subheader__breadcrumbs-link kt-subheader__breadcrumbs-link--active">Pembayaran Insentif</span>
			</div>
		</div>
	</div>
</div>
	...
	<div class="kt-portlet__head kt-portlet__head--lg">
		<div class="kt-portlet__head-label">
			...
			<h3 class="kt-portlet__head-title">
				Tabel Pembayaran Insentif
			</h3>			
			<div class="kt-portlet__head-toolbar">
				<div class="kt-portlet__head-wrapper">
					<div class="kt-portlet__head-actions">
						<a href="{{ route('pembayaran_insentif.create') }}">
...
$(document).ready(function () {
		var t = $('#kt_table').DataTable({
			...
			ajax      : {
						url: "{{ route('pembayaran_insentif.search.index') }}",
						type : "POST",
						dataType : "JSON",
						headers: {
			...
			$('#editRow').click(function(e) {
				...
				if($('input[type=radio]').is(':checked')) { 
					$("input[type=radio]:checked").each(function(){
						...
						location.replace("{{url('perbendaharaan/pembayaran_insentif/edit')}}"+ '/' +nodok);
					});
				} else {
					swalAlertInit('ubah');
				}
			...
			if($('input[type=radio]').is(':checked')) { 
				$("input[type=radio]:checked").each(function() {
					...
					const swalWithBootstrapButtons = Swal.mixin({
						...
						if (result.value) {
							$.ajax({
								url: "{{ route('pembayaran_insentif.delete') }}",
								type: 'DELETE',
								dataType: 'json',
								data: {
									"nodok": nodok,
