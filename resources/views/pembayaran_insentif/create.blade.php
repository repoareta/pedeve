...
$('#form-create').submit(function(){
	...
	$.ajax({
		url  : "{{route('pembayaran_insentif.store')}}",
		type : "POST",
		...
		if(data == 1){
			Swal.fire({
				...
				location.replace("{{url('perbendaharaan/pembayaran_insentif/edit')}}"+ '/' +scurrdoc);
				});
		}else if(data = 2){
			Swal.fire({
				type  : 'info',
