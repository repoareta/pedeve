...
class PencapaianKinerjaController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = DB::select("select a.*, b.nama,b.id, c.* from tbl_monitoring a join cm_perusahaan_afiliasi b on a.kd_perusahaan=b.id, tbl_rencana_kerja c  where a.kd_perusahaan=c.kd_perusahaan and a.tahun='$tahun'");
        return view('pencapaian_kerja.index',compact('data'));
    }

    public function search(Request $request)
    {          
        $data =DB::select("select a.*, b.nama,b.id, c.* from tbl_monitoring a join cm_perusahaan_afiliasi b on a.kd_perusahaan=b.id, tbl_rencana_kerja c  where a.kd_perusahaan=c.kd_perusahaan and  a.tahun='$request->tahun'");
        return view('pencapaian_kerja.index',compact('data'));
